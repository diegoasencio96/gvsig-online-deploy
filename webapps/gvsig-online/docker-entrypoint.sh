...
python /srv/gvsig-online-deploy/gvsig-online/gvsigol/manage.py compilemessages -l es --settings=tourism.settings.testing
python /srv/gvsig-online-deploy/gvsig-online/gvsigol/manage.py migrate --settings=tourism.settings.testing                 # Apply database migrations
...
exec gunicorn gvsigol.wsgi:application \
    --env DJANGO_SETTINGS_MODULE=gvsigol.settings.testing \
